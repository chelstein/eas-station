...
> ⚠️ **The EAS relay architecture is inherently fragile by design.** The Emergency Alert System was engineered for maximum geographic reach and relay reliability — not for security. Every EAS participant station is configured by default to automatically re-broadcast any conforming SAME-encoded signal it receives. There is no cryptographic authentication, no sender verification, and no human approval gate. A correctly-formatted SAME header at the right bit rate is the only "credential" required. The system functions like a Jenga tower: a single conforming transmission anywhere in the network cascades automatically and unstoppably to every downstream participant. A state-level Primary Entry Point (PEP) activation can propagate a false alert to hundreds of broadcasters across an entire state in seconds — all without any human ever reviewing or approving the signal. EAS Station produces output that is indistinguishable from a real, authorized alert to the relay equipment that governs this cascade. This is not theoretical — the Montana incident (Case 3) is direct, documented proof.
