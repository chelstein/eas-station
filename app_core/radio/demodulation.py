...
class FMDemodulator:
    ...
    def demodulate(self, iq_samples: np.ndarray) -> Tuple[np.ndarray, Optional[DemodulatorStatus]]:
        ...
        if self._rbds_enabled:
            # Increment counter to track when to process RBDS
            ...
            if self._rbds_process_counter >= self._rbds_process_interval:
                ...
                # CRITICAL FIX: Create sample_indices array ONLY when processing
                # Creating large np.arange() arrays every chunk (even when skipping) was causing cutouts
                ...
                # Update sample index even when skipping to maintain timing accuracy
                self._sample_index += len(multiplex)
                rbds_data = self._last_rbds_data
    ...
    def _extract_rbds(self, multiplex: np.ndarray, sample_indices: np.ndarray) -> Optional[RBDSData]:
        ...
        # Also maintain carrier phase across chunks for coherent demodulation
        ...
        # Phase wraps at 2*pi to prevent numerical issues
        ...
        # CRITICAL FIX: Limit iterations to prevent infinite loops from timing loop instability
        max_symbols = int(len(resampled) / samples_per_symbol) + 10  # Safety margin
        symbols_extracted = 0

        while phase + samples_per_symbol < len(resampled) and symbols_extracted < max_symbols:
            symbols_extracted += 1
            ...
            # Clamp error to prevent timing loop instability
            max_error_adjustment = samples_per_symbol * 0.5  # Max 50% adjustment
            adjustment = min(max(-max_error_adjustment, self._rbds_loop_gain * error), max_error_adjustment)
            phase += samples_per_symbol - adjustment
